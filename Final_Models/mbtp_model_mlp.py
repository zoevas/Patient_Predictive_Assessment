"""
Created on Tue Jan 16 11:31:13 2024
...
for scaler, scaler_name in zip(sc_list,sc_names):
    for sampler, sampler_name in zip(imb_list,imb_names):
        ...
        hidden_layer_sizes_l = [(100,)]
        activation_l = ['tanh']
        alpha_l = [0.07]
        solver_l = [ 'sgd']
        for hidden in  hidden_layer_sizes_l:
            for act in activation_l:
                for al in alpha_l:
                    for solv in solver_l:
                        ...
                        print(' ', scaler_name,' ',sampler_name,' ',hidden,' ',act,' ',al,' ',solv)
                        print('\nOverall f1 macro:',
...
x_scaled = scaler_standard.fit_transform(x_all)
...
explainer = shap.KernelExplainer(model=clf.predict_proba, data=x_train_1[:150,:])

pickle.dump(explainer, open('shap_models/shap_explaine_mbtp.pkl', "wb"))
...
with open('models/mlp_for_mbtp.pkl', 'wb') as model_file:
    pickle.dump(clf, model_file)
print('load')    
with open('models/mlp_for_mbtp.pkl', 'rb') as model_file:
    loaded_model = pickle.load(model_file)    
   
with open('shap_models/shap_explaine_mbtp.pkl', 'rb') as model_file:
